<x-app-layout>
    ...
    <style>
        .table-responsive tbody tr:nth-child(odd) { background-color: #f9f9f9; }
        .table-responsive tbody tr:nth-child(even) { background-color: #e9ecef; }
    </style>
                                ...
                                <tbody>
                                    @forelse($exchanges as $exchange)
                                        <tr class="text-center">
                                            <td>
                                                @if($exchange->user_id === auth()->id() || auth()->user()->hasRole('admin'))
                                                    ...
                                                    @if(auth()->user()->hasRole('admin'))
                                                        <button class="btn btn-sm btn-danger delete-exchange-btn" data-id="{{ $exchange->id }}">
                                                            حذف
                                                        </button>
                                                    @endif
                                                @else
                                            ...
                                            <td class="font-bold text-center align-middle h-16">{{ $exchange->user->name ?? 'غير معروف' }}</td>
                                            <td class="text-primary font-bold text-center align-middle h-16">{{ $exchange->transaction_code }}</td>
                                            <td class="font-bold text-center align-middle h-16">{{ $transaction[$exchange->transaction_type] ?? $exchange->transaction_type }}</td>
                                            ...
                                            <td class="font-bold text-center align-middle h-16">{{ $exchange->rate }}</td>
                                            <td class="font-bold text-center align-middle h-16">{{ $exchange->note }}</td>
                                            <td class="font-bold text-center align-middle h-16">{{ $exchange->created_at->format('Y-m-d') }}</td>
                                        </tr>
                                    @empty
                                        <tr>
